...
export default function KanbanPage() {
  ...
  useEffect(() => {
    ...
    board.addEventListener('wheel', scrollWithWheel, { passive: false })
    return () => board.removeEventListener('wheel', scrollWithWheel)
  }, [loading])
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    {!loading && <div className="mb-3 flex items-center justify-end gap-2"><span className="mr-auto text-xs text-slate-500 dark:text-slate-400">Hover over the board and use your mouse wheel to scroll sideways.</span><button onClick={() => scrollBoard(-1)} aria-label="Scroll board left" className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600 transition hover:border-blue-300 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"><ChevronLeft size={18} /></button><button onClick={() => scrollBoard(1)} aria-label="Scroll board right" className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600 transition hover:border-blue-300 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"><ChevronRight size={18} /></button></div>}
