...
function normalizeCompletion(task, previousStatus) { 
...
export const taskService = {
  ...
  async create(user, payload) { const task = await taskRepository.create({ ...payload, createdBy: user.id, assignedUser: payload.assignedUser || user.id }); normalizeCompletion(task, null); await task.save(); await activityService.record({ actor: user.id, entityType: 'task', entityId: task.id, action: 'task_created', message: `Created task: ${task.title}` }); return task },
  async update(user, taskId, payload) { 
    const task = await this.get(user, taskId); 
    const previous = task.toObject(); 
    const previousStatus = task.status; 
    Object.assign(task, payload); 
    normalizeCompletion(task, previousStatus); 
    const changes = recordChanges(previous, task.toObject()); 
    console.log('Task update - previousStatus:', previousStatus, 'newStatus:', task.status, 'completionDate:', task.completionDate)
    await taskRepository.update(task); 
    if (Object.keys(changes).length) 
      await activityService.record({ actor: user.id, entityType: 'task', entityId: task.id, action: task.status === 'completed' && previousStatus !== 'completed' ? 'task_completed' : 'task_updated', changes, message: `Updated task: ${task.title}` }); 
    return task 
  },
