...
const taskBody = z.object({ title: z.string().trim().min(1).max(160), description: z.string().trim().max(5000).optional(), project: nullableObjectId, category: nullableObjectId, boardList: nullableObjectId, priority: z.enum(TASK_PRIORITIES).optional(), status: z.enum(TASK_STATUSES).optional(), assignedUser: nullableObjectId, dueDate: z.coerce.date().nullable().optional(), reminderDate: z.coerce.date().nullable().optional(), estimatedTime: z.coerce.number().min(0).nullable().optional(), actualTime: z.coerce.number().min(0).nullable().optional(), tags: z.array(z.string().trim().min(1).max(40)).max(20).optional() })
