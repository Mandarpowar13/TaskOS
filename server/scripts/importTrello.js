...
if (!exportPath || !userEmail) {
  throw new Error(
    "Usage: node scripts/importTrello.js <trello-export.json> <user-email> [--apply]",
  );
}
...
const statusByListName = new Map([
  ...
  ["bugs raised", "todo"],
  ["wip", "in-progress"],
  ["pending from synargics", "waiting"],
  ["under review", "review"],
  ["done", "completed"],
]);
...
const cards = (board.cards || []).filter(
  (card) => !card.closed && activeListById.has(card.idList),
);
...
try {
  ...
  const cardsToImport = cards.filter(
    (card) => !existingTitles.has(card.name.trim().toLowerCase()),
  );
  const skippedCards = cards.filter((card) =>
    existingTitles.has(card.name.trim().toLowerCase()),
  );
  ...
  if (!apply) {
    ...
    const currentLists = await BoardList.find({ owner: user._id }).sort({
      position: 1,
    });
    const listByName = new Map(
      currentLists.map((list) => [list.name.trim().toLowerCase(), list]),
    );
    ...
    for (const sourceList of activeLists) {
      ...
      if (!listByName.has(key)) {
        const list = await BoardList.create({
          name: sourceList.name.trim(),
          status: statusForList(sourceList.name),
          position: position++,
          owner: user._id,
        });
